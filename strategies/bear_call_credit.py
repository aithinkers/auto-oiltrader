...
Sells a call closer to the money (~25Δ), buys a further-OTM call as wing.
Net credit. Bearish bias: profits if underlying stays below the short strike
at expiry.
"""
...
from core.verticals import find_credit_vertical, is_mean_reversion_short_setup
from strategies.base import Strategy, StrategySignal


class BearCallCredit(Strategy):
    ...
    def evaluate(self, market_state: dict, current_positions: list[dict]) -> list[StrategySignal]:
        ...
        if snap is None or snap.empty:
            return []
        if not is_mean_reversion_short_setup(snap):
            return []
        ...
        candidate = find_credit_vertical(
            ...
            right="C",
            target_short_delta=float(self.params.get("short_call_delta", 0.25)),
        ...
        thesis = (
            f"BearCallCredit: ATM IV {candidate.atm_iv*100:.1f}% on {candidate.trading_class} {candidate.expiry}, "
            f"sell {candidate.short_strike:.0f}C, buy {candidate.long_strike:.0f}C wing "
            f"(width=${candidate.width:.0f}, credit=${-candidate.net_amount:.2f}). "
            f"DTE={candidate.dte}. Spot={candidate.spot:.2f}. Profits if CL stays below breakeven."
        )
